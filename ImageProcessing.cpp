...
void imageProcessing(string input) {
    ...
    int select = -1 , plate_width, plate_height, count, friend_count = 0, refinery_count = 0;
    ...
    image = imread(input);
    imwrite("carImage/capturedImage.jpg", image);
    ...
    rectangle(image3, boundRect2[select].tl(), boundRect2[select].br(), Scalar(0, 255, 255), 2, 8, 0);
    //imshow("FullSnakeHead", image3);
    ...
    adaptiveThreshold(cropped_image, cropped_image, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY_INV, 23, 9);
    //imshow("AdaptiveThreshold", cropped_image);
...
string printCarNumber(string input) {
    const char* ch_input;
    ch_input = input.c_str();
    Mat image;
...
string getCarNumber(string text) {
    ...
    // ? doesn't exist in carplate
    text.erase(remove(text.begin(), text.end(), ' '), text.end());
    for (int j = 0; j < text.size(); j++) {
        string temp(1, text[j]);
        if (temp.compare(questionMark) == 0)
            text[j] = '9';
    }
    cout << text << '\n';
